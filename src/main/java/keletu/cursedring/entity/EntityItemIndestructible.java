...
public class EntityItemIndestructible extends EntityItem {
    ...
    protected static final DataParameter<Optional<UUID>> OWNER_UNIQUE_ID = EntityDataManager.<Optional<UUID>>createKey(EntityItemIndestructible.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        if (!this.world.isRemote) {
            ...
            if (allowPickUp) {
                ...
                if (!CursedRingMod.soulCrystal.retrieveSoulFromCrystal(player, itemstack))
                    return;

                CursedRingMod.packetInstance.sendToAllAround(new PacketRecallParticles(this.posX, this.posY + (this.getEyeHeight() / 2), this.posZ, 48, false), new NetworkRegistry.TargetPoint(player.world.provider.getDimension(), this.posX, this.posY + (this.getEyeHeight() / 2), this.posZ, 64));
